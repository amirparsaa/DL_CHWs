...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        num_train = self.X_train.shape[0]
        dists = np.zeros((num_test, num_train))
        dists = np.sqrt(np.sum(np.square(X), axis = 1).reshape(X.shape[0],1) + np.sum(np.square(self.X_train), axis = 1).reshape(1, self.X_train.shape[0]) - 2 * np.matmul(X, self.X_train.T))
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            dists_ind = np.argsort(dists[i])
            for j in range(k):
                closest_y.append(self.y_train[dists_ind[j]])
